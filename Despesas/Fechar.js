...
function fecharExecute() {
	...
	// Build Conta Corrent records
	// const despesasFecharTab 	 = despesasSpreadSheet.getSheetByName("Fechar");
	// const FecharAssociadoRange   = despesasFecharTab.getRange("FecharColaborador");
	// const FecharEstadiaRange     = despesasFecharTab.getRange("FecharEstadia");
	// const FecharDadosRange       = despesasFecharTab.getRange("FecharDados");
	// const FecharComentarioRange  = despesasFecharTab.getRange("FecharComentario");
	...
	if (debitoReal > 0) {
		...
		contaCorrenteRegistro = fillUpRegister (FecharData, FecharColaborador, FecharEstadia, 'Real', 'Credito', item, debitoReal, FecharComentario);
		contaCorrente.push(contaCorrenteRegistro)
	}

	if (debitoOuro > 0) {
		var item =  'OColaborador pagou a Mineração Carará seu debito em Ouro';
		contaCorrenteRegistro = fillUpRegister (FecharData, FecharColaborador, FecharEstadia, 'Real', 'Credito', item, debitoOuro, FecharComentario);
		contaCorrente.push(contaCorrenteRegistro)
	}
	...
	// Clear form
	FecharColaboradorRange.setValue("");
